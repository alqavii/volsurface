...
class surface():
    ...
    def printDividendYield(self):
        dividend_yield = self.getDividendYield()
        print(f"Dividend Yield for {self.ticker}: {dividend_yield:.2f}%\n")
        return dividend_yield
    ...
    #    rate = self.latestRate / 100
    #    dividendYield = self.getDividendYield()/100
    #    vars = self.calls[['strike', 'lastPrice']].rename(columns={'lastPrice': 'call_last'})
    #    #for expiry, data in vars.items():
    #    return 

    def _blackScholesCall(self, sigma, K, T):
        r = self.latestRate / 100
        q = self.getDividendYield() / 100
        S = self.spot
...
if ticker:
    ...
    st.write(f"• Dividend yield: {surf.getDividendYield()*100:.2f}%")
